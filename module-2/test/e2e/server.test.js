...
describe('End2End API Suite test', () => {
  ...
  describe('/calculateFinalPrice:post', () => {
    it('given a carCategory, customer and numberOfDays it should calculate final amount in real', async () => {
      ...
      const response = await request(app.server).post('/calculateFinalPrice').send(body).expect(200)
  ...
  describe('rent:post', () => {
    it('given a customer and a car category it should return a transaction receipt', async () => {
      ...
      expect(result.amount).to.not.be.deep.empty
      expect(result.dueDate).to.not.be.deep.empty
    })
  })
})
